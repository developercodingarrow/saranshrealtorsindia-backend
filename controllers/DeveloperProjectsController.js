...
exports.createDeveloperPage = catchAsync(async (req, res, next) => {
  const { pageTitle, developer, topDescription, bottomDescription } = req.body;
  ...
  resultStatus(res, 201, "created new Developer page", saveNewDeveloperProject);
});
...
exports.deletePormotionalPage = catchAsync(async (req, res, next) => {
  const { _id } = req.body;
  console.log(_id);
  const data = await DeveloperProjects.findByIdAndDelete(_id);
  resultStatus(res, 200, "Developer Deleted", data);
});
